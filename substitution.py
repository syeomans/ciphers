...
def randSubstitution(inputFileName, outputFileName, preserveCaps=True, preservePunctuation=True):
...
def encodeSubstitution(inputFileName, outputFileName, cipherAlphabet, preserveCaps=True, preservePunctuation=True):
    # Open all relevant files
    inFile = open(inputFileName, 'r')
    outFile = open(outputFileName, 'w')
    cipherAlphabetFile = open('cipherAlphabet.txt', 'w')
...
def decodeSubstitution(inputFileName, outputFileName, cipherAlphabet):
    # Open all relevant files
    inFile = open(inputFileName, 'r')
    outFile = open(outputFileName, 'w')
...
# Test script
randSubstitution('plaintext.txt', 'ciphertext.txt')
cipherAlphabet = open('cipherAlphabet.txt').read()
decodeSubstitution('ciphertext.txt', 'decoded plaintext.txt', cipherAlphabet)
