# This script encrypts the user's message with a sliding caesar cipher. If the user
# has an encoded message, it will decode the message. If not, it will encode a new one.
# The cipher increments the ASCII values of each character of text by an incrementing counter.


# Encrypt message with sliding Caesar cipher. Scrambles text by incrementing the ASCII values of the text.
def encode(inStr):
	outStr = ""
	count = 0
	for letter in inStr:
